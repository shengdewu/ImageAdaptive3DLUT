...
cv::Mat Lut::trilinear(const cv::Mat &src, const cv::Mat &lut) {
    ...
    output.forEach<cv::Vec3f>([&](cv::Vec3f &rgb, const int position[]) -> void {
        ...
        const int x100 = r1 + (b0 % cell) * len;
        const int y100 = g0 + (b0 / cell) * len;
        const int x010 = r0 + (b0 % cell) * len;
        const int y010 = g1 + (b0 / cell) * len;
        const int x110 = r1 + (b0 % cell) * len;
        const int y110 = g1 + (b0 / cell) * len;
        const int x001 = r0 + (b1 % cell) * len;
        const int y001 = g0 + (b1 / cell) * len;
        const int x101 = r1 + (b1 % cell) * len;
        const int y101 = g0 + (b1 / cell) * len;
        const int x011 = r0 + (b1 % cell) * len;
        const int y011 = g1 + (b1 / cell) * len;
        const int x111 = r1 + (b1 % cell) * len;
        const int y111 = g1 + (b1 / cell) * len;

        rgb = linearInterp(lut.at<cv::Vec3f>(y000, x000),
...
cv::Mat Lut::trilinear_forward(const float *lut, const cv::Mat &image, const int dim) {
    ...
    for (int index = 0; index < output_size; ++index) {
        int offset = index * 3;
        float r = image_ptr[offset];
        float g = image_ptr[offset + 1];
        float b = image_ptr[offset + 2];
        ...
        out_ptr[offset] = w000 * lut[id000] + w100 * lut[id100] +
                          w010 * lut[id010] + w110 * lut[id110] +
                          w001 * lut[id001] + w101 * lut[id101] +
                          w011 * lut[id011] + w111 * lut[id111];

        out_ptr[offset + 1] = w000 * lut[id000 + shift] + w100 * lut[id100 + shift] +
        ...
        out_ptr[offset + 2] = w000 * lut[id000 + shift * 2] + w100 * lut[id100 + shift * 2] +
        ...
        for (int by = 0; by < cell_size; by++) {
            for (int g = 0; g < lut_dim; g++) {
                for (int r = 0; r < lut_dim; r++) {
                    ...
                    auto lut_loc = r + g * g_offset + b_offset * b;
                    float lut_r = lut_prt[lut_loc];
                    float lut_g = lut_prt[lut_loc + offset];
                    float lut_b = lut_prt[lut_loc + offset * 2];

                    lut_mat.at<cv::Vec3f>(y, x)[2] = lut_b;
                    lut_mat.at<cv::Vec3f>(y, x)[1] = lut_g;
                    lut_mat.at<cv::Vec3f>(y, x)[0] = lut_r;
                }
            }
        }
    }
