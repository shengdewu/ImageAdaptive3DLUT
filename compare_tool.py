...
class CompareBase:
    ...
    def compare(self, base_path, compare_paths, out_path, skip=False, special_name=None, img_format='jpg'):
        if base_path != '' and not os.path.exists(base_path):
            print('the compare base path {} not exists'.format(base_path))
            return

        compare_exists = [not os.path.exists(path) for path in compare_paths]
        if np.any(compare_exists):
            print('the compare path {} not exists'.format(compare_paths))
            return
        ...
        base_names = [name for name in os.listdir(base_path) if name.lower().endswith(img_format)]
...
class Compare(CompareBase):
    ...
    def execute(self, shapes, base_img, compare_imgs):
        ...
        for i in range(len(compare_imgs)):
            h, w, c = shapes[i]
            assert shapes[i] == compare_imgs[i].shape
            concat[:h, start_w:start_w+w, :] = compare_imgs[i]
            start_w += w
        return concat